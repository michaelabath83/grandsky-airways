...
function setupFlightsManager() {
  ...
  try {
    onSnapshot(col, snap => {
      ...
      showToast('Could not load flights (realtime). Using fallback.', 'error');
      // fallback to static refresh
      getDocs(col).then(snap => renderFlightsTable(snap.docs.map(d=>({ id:d.id, ...d.data() })))).
        catch(e => console.error('fallback getDocs failed', e));
    });
  } catch (e) {
    console.error('setupFlightsManager onSnapshot failed', e);
    try { getDocs(col).then(snap => renderFlightsTable(snap.docs.map(d=>({ id:d.id, ...d.data() })))); } catch(_){ }
  }
...
function loadAirports() {
  ...
  try {
    onSnapshot(col, snap => {
      ...
      console.error('airports onSnapshot error', err);
      // fallback
      getDocs(col).then(snap => renderGenericTable(wrap, snap.docs.map(d=>({ id:d.id, ...d.data() })), ['code','city','country','name','region'], 'airports'))
        .catch(e => console.error('fallback airports getDocs failed', e));
    });
  } catch (e) {
    try { getDocs(col).then(snap => renderGenericTable(wrap, snap.docs.map(d=>({ id:d.id, ...d.data() })), ['code','city','country','name','region'], 'airports')); } catch(_){ }
  }
}
...
function loadAirlines() {
  ...
  try {
    onSnapshot(col, snap => {
      ...
      console.error('airlines onSnapshot error', err);
      getDocs(col).then(snap => renderGenericTable(wrap, snap.docs.map(d=>({ id:d.id, ...d.data() })), ['code','name','color'], 'airlines'))
        .catch(e => console.error('fallback airlines getDocs failed', e));
    });
  } catch (e) {
    try { getDocs(col).then(snap => renderGenericTable(wrap, snap.docs.map(d=>({ id:d.id, ...d.data() })), ['code','name','color'], 'airlines')); } catch(_){ }
  }
}
...
function setupPendingPayments() {
  ...
  // Try real-time snapshot; if it fails, fallback to polling + static load
  try {
    let unsubscribe = null;
    try {
      unsubscribe = onSnapshot(q, snapshot => {
        // on successful snapshot, clear any polling
        if (window.__pendingPaymentsPoll) { clearInterval(window.__pendingPaymentsPoll); window.__pendingPaymentsPoll = null; }
        ...
        // start polling every 8 seconds as a fallback to get near-real-time updates
        if (!window.__pendingPaymentsPoll) {
          window.__pendingPaymentsPoll = setInterval(fallbackLoadPendingPayments, 8000);
        }
      });
    } catch (innerErr) {
      console.error('onSnapshot failed to subscribe:', innerErr);
      showToast('Real-time subscription unavailable. Using fallback mode.','error');
      fallbackLoadPendingPayments();
      if (!window.__pendingPaymentsPoll) window.__pendingPaymentsPoll = setInterval(fallbackLoadPendingPayments, 8000);
    }
  } catch (err) {
...
function renderPendingPaymentsTable(rows) {
    ...
    tr.innerHTML = `
      ...
      <td>
        <div style="display:flex;gap:6px;align-items:center;justify-content:flex-end">
          <button class="approve-small" data-id="${r.id}"
            style="background:var(--success);color:#fff;padding:6px 10px;border-radius:6px;border:none;font-weight:700;font-size:13px">✓</button>
          <button class="reject-small" data-id="${r.id}"
            style="background:var(--danger);color:#fff;padding:6px 10px;border-radius:6px;border:none;font-weight:700;font-size:13px">✕</button>
        </div>
      </td>
    `;
    body.appendChild(tr);
  ...
  wrap.querySelectorAll('.approve-small').forEach(b => b.addEventListener('click', async (e) => {
    ...
    try {
      ...
      await approvePayment(bookingId, adminEmail);
  ...
  // Keep old review-panel wiring removed (we now use inline buttons)
}
...
function loadUsers() {
  ...
  try {
    onSnapshot(col, snap => {
      ...
      getDocs(col).then(snap => renderGenericTable(wrap, snap.docs.map(d=>({ id:d.id, ...d.data() })), ['firstName','lastName','email','role'], 'users'))
        .catch(e => console.error('fallback users getDocs failed', e));
    });
  } catch (e) {
    try { getDocs(col).then(snap => renderGenericTable(wrap, snap.docs.map(d=>({ id:d.id, ...d.data() })), ['firstName','lastName','email','role'], 'users')); } catch(_){ }
  }
}
