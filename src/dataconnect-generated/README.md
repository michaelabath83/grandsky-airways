...
## SearchFlights
You can execute the `SearchFlights` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
searchFlights(vars: SearchFlightsVariables, options?: ExecuteQueryOptions): QueryPromise<SearchFlightsData, SearchFlightsVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
searchFlights(dc: DataConnect, vars: SearchFlightsVariables, options?: ExecuteQueryOptions): QueryPromise<SearchFlightsData, SearchFlightsVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the searchFlightsRef:
```typescript
const name = searchFlightsRef.operationName;
console.log(name);
```

### Variables
The `SearchFlights` query requires an argument of type `SearchFlightsVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `SearchFlights` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `SearchFlightsData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `SearchFlights`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, searchFlights, SearchFlightsVariables } from '@dataconnect/generated';

// The `SearchFlights` query requires an argument of type `SearchFlightsVariables`:
const searchFlightsVars: SearchFlightsVariables = {
  departure: ..., 
  arrival: ..., 
};

// Call the `searchFlights()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await searchFlights(searchFlightsVars);
// Variables can be defined inline as well.
const { data } = await searchFlights({ departure: ..., arrival: ..., });
...
// Or, you can use the `Promise` API.
searchFlights(searchFlightsVars).then((response) => {
  const data = response.data;
  console.log(data.flights);
});
```

### Using `SearchFlights`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, searchFlightsRef, SearchFlightsVariables } from '@dataconnect/generated';
...
// Call the `searchFlightsRef()` function to get a reference to the query.
const ref = searchFlightsRef(searchFlightsVars);
// Variables can be defined inline as well.
const ref = searchFlightsRef({ departure: ..., arrival: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = searchFlightsRef(dataConnect, searchFlightsVars);
...
// Variables can be defined inline as well.
const { data } = await createUser({ email: ..., fullName: ..., });
...
// Variables can be defined inline as well.
const ref = createUserRef({ email: ..., fullName: ..., });
...
// Variables can be defined inline as well.
const { data } = await createBooking({ flightId: ..., seatNumber: ..., });
...
// Variables can be defined inline as well.
const ref = createBookingRef({ flightId: ..., seatNumber: ..., });
